...
**Status:** Live after Round-1 multi-agent swarm (2026-08-05); content corrections Checkpoint G.1 (2026-08-05); Round-2 Fable solo blinded round (2026-08-10) recorded in §6a — buckets unchanged, two corrections applied (COR-0043, COR-0044).  
...
Confirmed/objective examples: extreme DXA phenotype, serial 24h urine calcium values (see CLM-0015 thresholds), IgG subclass/titer pattern, structural spine disease. Reported / narrative-grade (do **not** cite as confirmed; COR-0043): historical T~34 — clinical-note value only, discrete lab report not retrieved (T115; lowest documented draw is 140 ng/dL immunoassay); thiamine 7 nmol/L — single borderline value one unit below range, with a self-reported response; HαT positive narrative only — no TPSAB1 CNV instrument report (CLM-0031; UQ-0014).

Babesia satellite note (R2, 2026-08-10): the strongest single negative datum is the absent hematologic footprint — CBC and platelets normal across 3+ years, and LDH never documented high (low 2025, then normal 2026; T099) — against a hypothesis of chronic untreated intra-erythrocytic infection. And the FISH record forks (COR-0044): the 2024-02 positive **postdates** the mid-2023 antibiotic course, so either the first treatment response coexisted with persistent parasitemia (undermining an eradication-mediated read) or the organism evidence reduces to a single non-confirmed LDT result. What remains undone is a post-antibiotic **commercial-lab** reconfirmation, not a post-antibiotic FISH.
...
## 6a. Round-2 — Fable solo blinded round (2026-08-10)

Fourth-model review: Claude Fable 5, run solo (blinded re-derivation from the evidence pack only → adversarial cross-exam of both the blinded output and this ranking → chair synthesis with source-document re-verification). Full method and findings: [`swarm-runs/2026-08-10-fable-blinded/03_synthesis.md`](swarm-runs/2026-08-10-fable-blinded/03_synthesis.md). Single-model caveat applies throughout: blinding substitutes for, but does not equal, cross-family independence.

**Buckets and rank order unchanged.** Standing objections preserved the same way §6 preserves minority reports:

- **H2 medium challenged as generous:** a modest related-cohort panel yield (lit-0340, which by its own card carries no numeric figure onto public surfaces), a decades-long high-impact trauma history without peripheral fracture (absent from H2's contradicting list), and two acquired insults — the documented hypogonadal era and the newly wired, patient-reported aromatase-inhibitor exposure (CLM-0112, UQ-0020; the relevant cards sat uncited by any hypothesis since R1 — lit-0134 now joins H2's contradicting list, with lit-0308 carried as its short-horizon comparator). "Best single-primary *candidate to test*" stands; the panel (test #4) remains the discriminator.
- **H-HαT "rejected as driver" challenged as one notch too absolute:** the genotype is narrative-only (CLM-0031, UQ-0014) and the trait-association literature is contested both ways; defensible driver bucket is **speculative**. (The blinded stage's low–medium was refuted as too generous for the same reasons.)
- **H1 vs H-NULL discrimination gap:** no listed test separates "interacting layers" from "independent coexistence"; next full round should name a discriminating observation or merge the entries.
- **Independent re-derivations that reinforce this page:** multi-process architecture; tick-borne low with the §5 channel split; SM don't-miss with hs KIT D816V; SFN untested with IENFD as the top-value test; H3's refusal to classify the hypercalciuria mechanism (the blinded stage over-called "resorptive"; the adversarial stage refuted it from the primary numbers back to exactly H3's position); psychiatric not-supported.
- **Proposed don't-miss/test candidates pending adjudication:** T. whipplei testing; plasma/erythrocyte porphyrins (new 1–2-minute photosensitivity); quantitative sulfur amino acids; naming idiopathic osteoporosis of the young male — the residual clinical label under which treatment cohorts such as lit-0008 were enrolled — as the bucket this phenotype enters when secondary workup stays negative, sharpening why formation markers are test #1; characterizing the polyuria/possible-volume-contraction discrepancy; identifying the childhood IIH diuretic (acetazolamide question) and the vitamin-A axis.
