...
### COR-0043 — The public ranking's "Confirmed/objective" row carried two items that are not confirmed or objective

- **Date identified:** 2026-08-10
- **Status:** `applied_to_public_draft`
- **Severity:** should_fix_before_launch
- **Affected claims / paths:** `differentials/current_ranking.md` §5 ("Specialty LDT vs confirmed"); no claim-inventory rows (the inventory itself already carried the correct provenance — the inconsistency was between the ranking page and the project's own sources)
- **Incorrect or unsafe form:** The §5 list of "Confirmed/objective examples" included "historical T~34" and "thiamine 7 nmol/L with response", presenting a clinical-note-only testosterone value and a single borderline thiamine result bundled with an unblinded self-reported response as confirmed, objective anchors
- **Corrected form:** §5 now separates the row: confirmed/objective items (DXA phenotype, serial 24-h urine calcium, IgG subclass/titer pattern, structural spine disease) are listed apart from reported/narrative-grade items ("historical T~34 — clinical-note value only, discrete lab report not retrieved (T115); thiamine 7 nmol/L — single borderline value one unit below range, with a self-reported response"), matching the register the claim inventory already used
- **Evidence:** The endocrine summary's methodology note (1) states the ~34 and 210 ng/dL values are "recorded in a clinical note only; assay and units as documented in the chart, not from a discrete lab report"; the lowest documented draw is 140 ng/dL by an immunoassay whose lab flags reduced accuracy below 250. Ledger item T115 ("Full HPG at original T=34 — Incomplete") already tracked the missing retrieval. The thiamine 7 nmol/L (ref 8–30) is a single serum draw; the attributed response is self-reported with ~12-hour onset (video transcript ~00:21:36). Found by the 2026-08-10 Fable solo blinded round (adversarial stage, finding B-1), chair-verified against the source PDFs
- **Residual uncertainty:** None about the provenance labels themselves. Whether the historical testosterone was truly ~34 ng/dL remains open until the original laboratory reports are retrieved (T115; acquisition asks)

### COR-0044 — The tests ledger recorded the post-antibiotic Babesia FISH as never done when its own T047 row contains one

- **Date identified:** 2026-08-10
- **Status:** `logged_in_inventory`
- **Severity:** should_fix_before_launch
- **Affected claims / paths:** private tests ledger rows T113 and S008 (`evidence/facts/tests_ledger.md`/`.yaml`, private archive; not rendered publicly); the infection-adjudication framing in `differentials/current_ranking.md` §5 note (public)
- **Incorrect or unsafe form:** T113 "Post-abx Babesia FISH — Not documented" and S008 "Post-abx Babesia FISH + commercial reconfirm — Specialty+ never clearly retested", implying the adjudicating retest the infection question needs was never performed
- **Corrected form:** T047's second row **is** a post-antibiotic FISH: the 2024-02 positive followed the mid-2023 four-week atovaquone+clindamycin course. The ledger rows now carry a correction note; the ranking's satellite note states the resulting fork: if the post-treatment positive is credited, the first treatment response coexisted with persistent parasitemia on the definitive regimen (undermining an eradication-mediated read of the response); if it is discounted, the organism evidence reduces to a single non-confirmed specialty-LDT result. Either branch lowers the weight "two positives" carries intuitively. What remains genuinely not documented is a post-antibiotic **commercial-lab** reconfirmation attempt
- **Evidence:** Infectious-disease summary p2 (FISH positives 2023-07 and 2024-02; same-panel PCR/immunoblots negative) and p6 (treatment courses: atovaquone+clindamycin ×4 weeks mid-2023; +azithromycin ×6 weeks 2024). Found by the 2026-08-10 Fable solo blinded round (adversarial stage, finding B-5 / shared-blind-spot 2), chair-verified against the ledger and the summary
- **Residual uncertainty:** The FISH assay is a non-FDA-cleared specialty LDT; nothing in this correction upgrades or downgrades its analytic validity. The fork is an argument about internal consistency, not a new laboratory result
